...
/*   Updated: 2023/02/09 17:10:47 by sawang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static void	fdf_put_pixel(mlx_image_t *img, t_pixel pixel, uint32_t color, int sign)
{
	if (sign == 1)
	{
...
static void	drawlinelow(mlx_image_t *g_img, t_coord p1, t_coord p2, int sign)
{
	...
	while (p1.pixel.u++ <= p2.pixel.u)
	{
		...
		fdf_put_pixel(g_img, p1.pixel, p1.color, 1);
		// p1.pixel.u++;
		if (delta.p < 0)
