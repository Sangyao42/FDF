...
static void	drawlinelow(mlx_image_t *g_img, t_pixel p1, t_pixel p2)
{
	...
	while (p1.u <= p2.u)
	{
		fdf_put_pixel(g_img, p1, 0x00FF00FF);
		p1.u++;
		if (p < 0)
			p = p + 2 * dy;
		else
...
static void	drawlinelow2(mlx_image_t *g_img, t_pixel p1, t_pixel p2)
{
	int			dx;
	int			dy;
	int			yi;
	...
	while (p1.u <= p2.u)
	{
		fdf_put_pixel(g_img, p1, 0xFF0000FF);
		p1.u++;
		if (p < 0)
			p = p + 2 * dy;
		else
...
static void	drawlinehigh2(mlx_image_t *g_img, t_pixel p1, t_pixel p2)
{
	int			dx;
	int			dy;
	int			xi;
...
void	draw_line2(mlx_image_t *g_img, t_pixel p1, t_pixel p2)
{
	...
	{
		if (p1.u > p2.u)
			drawlinelow2(g_img, p2, p1);
		else
			drawlinelow2(g_img, p1, p2);
	}
	else
	{
		if (p1.v > p2.v)
			drawlinehigh2(g_img, p2, p1);
		else
			drawlinehigh2(g_img, p1, p2);
	}
}
